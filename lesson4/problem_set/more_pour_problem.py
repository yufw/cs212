    ...
    def successors(s):
        ...
        for i in range(N):
            if s[i] < capacities[i]:
                ...
                items.append((tuple(state), ('fill', i)))
            if s[i] > 0:
                state = list(s)
                state[i] = 0
                items.append((tuple(state), ('empty', i)))
        for pair in itertools.permutations(range(N), 2):
            i, j = pair
            if s[i] == 0 and s[j] == capacities[j]:
                continue
            if s[i] + s[j] <= capacities[j]:
                state = list(s)
                state[i], state[j] = 0, s[i] + s[j]
                items.append((tuple(state), ('pour', i, j)))
            else:
                state = list(s)
                state[i], state[j] = s[i] - (capacities[j] - s[j]), capacities[j]
                items.append((tuple(state), ('pour', i, j)))
        return dict(items)
